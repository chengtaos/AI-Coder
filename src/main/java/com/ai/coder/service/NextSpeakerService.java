...
public class NextSpeakerService {
    ...
    private static final String CHECK_PROMPT = """
            根据你上一次的回复，下一步应该由谁发言：'user'（用户） 还是 'model'（模型）？
            规则：如果你表明接下来有行动或回复未完成 -> 'model'。
            如果你向用户提问 -> 'user'。
            如果任务已完成 -> 'user'。
            以 JSON 格式回复：{"next_speaker": "user/model", "reasoning": "简要原因"}
            """;
    ...
    private NextSpeakerResponse performLLMCheck(List<Message> conversationHistory) {
        try {
            ...
            // 添加检查提示
            checkMessages.add(new UserMessage(CHECK_PROMPT));
